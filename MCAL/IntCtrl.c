...
void IntCtrl_init(const IntCtrl_ConfigType* ConfigPtr)
{
  ...
  for(uint8 i = 0; i < INTCTRL_CONFIGURED_INTERRUPT ; i++)
  {
    ...
    */
                                                                       /*this calculation based in timer_0_A*/
    IntCtl_Number = IntCtrl_Configuration[i].IntCtrl_Interrupt_Number; /* 19 */
    EnableRegOffset=(IntCtl_Number/REG_LENGTH_BIT)*REG_LENGTH_BYTE;    /*  (19/32)*4 = 2.375 = 2 */
    OffsetBit=IntCtl_Number%REG_LENGTH_BIT;                            /*  19%32 = 19  */
    switch(EnableRegOffset/REG_LENGTH_BYTE)                            /* 2/4 = 0.5 = 0 */
    {
